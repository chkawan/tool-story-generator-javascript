/* =========================================
   VARIANTE ALTERNADA (foto + texto)

   Cópia de script-img.js: mesma lógica de
   posts, formato, painel de personalização e
   logo — só que em vez da grade 2x2 de 4
   cards, #postCards renderiza 3 linhas, cada
   uma com 1 foto (upload igual ao original,
   3 posições em vez de 4) e 1 "card" de texto
   em formato de pílula, alternando de lado a
   cada linha (ver style-img-sem-cards.css).
========================================= */
...
com.br" é gerado via ::after no
style-img-sem-cards.css (não muda por post),
então não tem elemento HTML nem o que
ler/escrever nele em JS.
*/
...
/* =========================================
   ...
   3 linhas: cada uma é [foto, texto] no
   markup — a linha do meio inverte visualmente
   (flex-direction: row-reverse no CSS), então
   o lado da foto alterna sozinho a cada linha.
   Usa só as 3 primeiras "dicas" do post (a
   variante original usa 4).
========================================= */
            ...
            (dica, cardIndex) => {
                ...
                */
                ...
                row.innerHTML = `
                    ...
                    <div class="card-photo" data-card-index="${cardIndex}">
...
/* =========================================
   ...
   Diferente da logo, as fotos são "por
   posição" (1, 2, 3) e valem pra qualquer
   post selecionado — é o mesmo modelo usado
   pelas fontes/cores do painel (configuração
   global, não por post).
========================================= */
    ...
    wraps.forEach(
        (wrap) => {
            ...
            if (dataUrl) {
                ...
                — ver comentário em .card-photo
                no style-img-sem-cards.css sobre
                o bug do html2canvas com
                object-fit.
                */
...
function initCardImageUpload() {
    ...
    /*
    Delegação de evento: as linhas são
    recriadas a cada renderPost(), então
    o listener fica no container fixo
    (#postCards), não em cada linha.
    */
...
const SETTINGS_FIELDS = [
    ...
    },
    {
        ...
        group: "Card — número",
        label: "Cor do texto (linhas ímpares)",
        type: "color",
        default: "#986DFF"
    },
...
/* =========================================
   GRUPO DAS FOTOS NO PAINEL

   O upload em si acontece clicando direto
   na foto, no preview (ver
   initCardImageUpload). Aqui só ficam dica +
   botões de remover, pra quando não quiser
   mexer no preview.
========================================= */
...
function buildCardImagesGroup() {
    ...
    group.innerHTML = `
        ...
        <p class="settings-field-hint">
            Clique em cada foto no preview pra
            enviar uma imagem (PNG, JPEG ou WEBP, até 5 MB).
            As fotos valem pela posição (1 a 3),
...
async function exportStory() {
    ...
    try {
        ...
        */
        ...
        /*
        Os botões "×" de remover (foto e logo)
        e o aviso "+ Adicionar imagem" são
        interface de edição, não fazem parte
        do post — tira tudo isso do clone antes
        de capturar pra não sair no PNG.
        */

        captureClone
            .querySelectorAll(
                ".card-photo-remove, .card-photo-placeholder, .hero-logo-remove"
            )
            .forEach(
                (element) => {
...
function createFilename(
    ...
    return `story-img-alternado-${String(id).padStart(2, "0")}-${cleanTitle}.png`;
